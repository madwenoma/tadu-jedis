...
public class CopyOfShardedJedisPool extends Pool<ShardedJedis> {
	public CopyOfShardedJedisPool(final GenericObjectPool.Config poolConfig,
			...
			for (Iterator<Entry<String, JedisStatus>> it = suspectJedises
				...
				if (jedisStatus.getFailDuration() > EJECT_ERROR_IN_TIME
					...
					for (Iterator<JedisShardInfo> shardIt = shards.iterator(); shardIt
						...
						if (getHostAndPort(shard).equals(hostAndPort)) {
							shardIt.remove();
